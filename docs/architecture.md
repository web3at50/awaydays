...
| `teaser.ts` | `teaserText`, which reduces Markdown link syntax to plain words for clamped card teasers (pure, tested) |
...
| `features.ts` | Which optional integrations have keys (`planSearchEnabled`, `tripadvisorEnabled`), so pages hide UI that can't work |
...
Diary: `RichText` (Markdown with `remark-gfm` + `remark-breaks` plus
...
npm run geocode:backfill            # coordinates for trips, entries and ideas that have a place but none
...
**Uppy v5** has no React `<Dashboard>` component, so mount the
