...
`npm run geocode:backfill` fills in trips and entries that have a location
but no coordinates, plus trip ideas that have an address but none.
...
`NEXT_PUBLIC_GOOGLE_MAPS_MAP_ID` are set; they then render
...
- Nominatim geocoding and OSRM road routing are **deliberately kept** even
  when Google renders the map, because Google's terms only allow caching their
...
on the standard grid, so Leaflet's default 256px boxes display them
retina-crisp.
...
skipped. Home is admin-editable in Settings. Unlike an entry, a geocode
...
`route_geometry`/`route_km` and drawn as a solid line. Everything else (train,
plane, ferry, hovercraft, walk) keeps a dashed straight arc, because
