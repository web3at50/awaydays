...
- **Clamped card teasers** (home page, shared index, the entry excerpt on a
  trip feed) stay flat. They are a taste of the text, not the text itself. A
...
the app. The geocode notice is the model: it says the entry was saved but the
place couldn't be found on the map, so the day isn't on the journey yet,
then suggests editing it with a fuller place name. Never surface a raw
error string to users.
